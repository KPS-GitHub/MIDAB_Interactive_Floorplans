...
class OptAttributeValueComponent extends React.Component<IOptAttributeValueComponentProps> {
    ...
    render() {
        ...
        nameString += " " + choice.quantity + " ";
        const qty = item.choice.getQtyUnit();
        if (qty != null) {
            nameString += " Qty " + qty.id;
        }
        ...
        return <div className={htmlClass}><select
            ...
            onChange={(ev) => this.handleChange(ev)}>
            {
                this.props.menu.items.map((item, idx) => {
                    //const price = this.props.pricingContext.getPriceDelta( val.choice);
                    const price = item.price;
                    return <OptAttributeValueComponent isSelected={!!self.props.selected && item.choice.isSameChoice(self.props.selected)}
                                                       index={idx}
                                                       price={price}
                                                       key={idx}
